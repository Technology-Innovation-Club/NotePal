...
class UploadTestCase(TestCase):
    ...
    def test_successful_upload_pdf(self):
        ...
        file = SimpleUploadedFile("test_file.pdf", self.test_file_content, content_type="application/pdf")
    ...
    def test_successful_upload_docx(self):
        ...
        file = SimpleUploadedFile("test_file.docx", self.test_file_content2, content_type="application/vnd.openxmlformats-officedocument.wordprocessingml.document")
    ...
    def test_successful_upload_pptx(self):
        ...
        file = SimpleUploadedFile("test_file.pptx", self.test_file_content3, content_type="application/vnd.openxmlformats-officedocument.presentationml.presentation")
    ...
    def test_upload_wrong_file_type(self):
        ...
        file = SimpleUploadedFile("test_file.jpg", self.test_file_content, content_type="image/jpeg")
        ...
        self.assertEqual(response.json(), {'detail': "{'File error': 'Invalid file type. Only pptx, docx, and pdf files are allowed.'}"})
    ...
    def test_upload_existing_file_name(self):
        ...
        duplicate_response = self.client.post(reverse("api-1.0.0:file_upload"), {"file": file})

        # Check if the response status code is 409 (Conflict)
        self.assertEqual(duplicate_response.status_code, 409)
        self.assertEqual(duplicate_response.json(), {'detail': "{'File error': 'File with the same name already exists.'}"})
